...
export class Field extends React.Component<Props, State> {
    ...
    handleSubmit(event: any) {
        this.setState({cells: this.generateCells(this.state.sizeX, this.state.sizeY)});
        event.preventDefault();
    }

    generateCells(x: number, y: number): Array<Array<CellStatus>> {
        let arr: Array<Array<CellStatus>> = [];
        for (let i = y; i > 0; i--) {
            arr[i] = [];
            for (let j = x; j > 0; j--) {
                ...
                <FieldContainer>
                    <Controls>
                        <form onSubmit={this.handleSubmit}>
                            <input type="number" min="10" value={this.state.sizeX} name="sizeX"
                                   onChange={this.handleChange}
                                   placeholder="X"/>
                            <input type="number" min="10" value={this.state.sizeY} name="sizeY"
                                   onChange={this.handleChange}
                                   placeholder="Y"/>
